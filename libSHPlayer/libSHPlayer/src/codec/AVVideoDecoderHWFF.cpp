...
AVVideoDecoderHWFF::AVVideoDecoderHWFF() {
...
void AVVideoDecoderHWFF::loadResource(const char *path) {
    filePath = path;
    avformat_context = avformat_alloc_context();
    AVInputFormat *fmt = NULL;
    AVDictionary *dic = NULL;
    int result = avformat_open_input(&avformat_context, path, fmt, &dic);
    if (result != 0) {
        char *error = NULL;
...
void AVVideoDecoderHWFF::decoderVideo() {
    //读取压缩数据AVPacket
...
//            AVVideoFrameData videoFrame;
//            videoFrame.stream_index = 0;
//            this->callback->onDecodeFrameReceived(callback->bridge, nullptr);
        }
    } else {
        if (ret == AVERROR_EOF) {
            isEOF = true;
...
void AVVideoDecoderHWFF::decoderPacket(AVCodecContext *dec_ctx, AVPacket *pkt, AVFrame *frame) {
    int ret;
    //7. 发送压缩数据给解码器
    ...
    while (ret >= 0) {
        ...
        if (frame->format == hw_pix_fmt) {
            ...
            videoFrame.plane_data[0] = frame->data[3];
...
void AVVideoDecoderHWFF::destroy() {
    /* 冲刷解码器 draining mode */
...
void AVVideoDecoderHWFF::decodeFrame() {
    if (isEOF) {
        printf("end of file");
    } else {
        decoderVideo();
 ...
 */
//硬解是否支持seek到任一点
void AVVideoDecoderHWFF::seekTo(float seekMs) {
...
void AVVideoDecoderHWFF::destroyHwAccel() {

}
void AVVideoDecoderHWFF::activateHwAccel() {
    destroy();
